...
from app.config import BASE_AMAZON_URL, HISTORY_MONTHS_COUNT, CURRENCY_CODE
...
def generate_22_month_history(base_price: float, mrp: float, end_date: datetime = None) -> List[Dict[str, Any]]:
    ...
    for i in range(HISTORY_MONTHS_COUNT - 1, -1, -1):
        ...
        if month_num in SALE_EVENTS:
            ...
            # 80% chance this product participated in this major sale
            if rng.random() < 0.85:
                is_sale = 1
                sale_tag = event_name
        ...
        # Ensure price never exceeds MRP and doesn't drop below 50% of MRP
        ...
        clean_price = round(float(clean_price), 2)
...
def seed_database_if_empty(force: bool = False):
    """Initializes and seeds database with the 25 verified Acer products & 22-month timelines."""
    init_db()
